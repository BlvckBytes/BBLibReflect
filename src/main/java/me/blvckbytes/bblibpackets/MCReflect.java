...
public class MCReflect {
  ...
  public MCReflect(
    ...
    this.reflClasses = new HashMap<>();

    this.ver = findVersion();
    this.refactoredNetwork = getVersion()[1] >= 17;
  }
  ...
  public Object getMinecraftServer() throws Exception {
    ...
    return getFieldByType(handle, getReflClass(ReflClass.MINECRAFT_SERVER), 0);
  }
   ...
   * @return Target field
   */
  public<T> Field findGenericFieldByType(Class<?> c, Class<?> type, @Nullable Class<T> genericType, int skip) throws Exception {
    return walkHierarchyToFind(c, cc ->
      Arrays.stream(cc.getDeclaredFields())
        .filter(it -> !Modifier.isStatic(it.getModifiers()))
        .filter(it -> type.isAssignableFrom(it.getType()))
        .filter(it -> genericType == null || it.getGenericType().getTypeName().contains(genericType.getSimpleName()))
        .skip(skip)
        .findFirst()
        .map(f -> {
          f.setAccessible(true);
  ...
  public Optional<Object> getNMSItem(ItemStack bukkitStack) {
    return getNMSStack(bukkitStack)
      .flatMap(is -> {
        try {
          return Optional.of(getFieldByType(is, getReflClass(ReflClass.ITEM), 0));
        } catch (Exception e) {
          logger.logError(e);
          return Optional.empty();
        }
  ...
  public Object getPlayerConnection(Player p) throws Exception {
    ...
    return findFieldByType(ep, getReflClass(ReflClass.PLAYER_CONNECTION), 0).get(ep);
  }
  ...
  public Object getNetworkManager(Player p) throws Exception {
    ...
    return findFieldByType(pc, getReflClass(ReflClass.NETWORK_MANAGER), 0).get(pc);
  }
  ...
  public boolean sendPacket(Player p, Object packet) {
    try {
      ...
      findMethodByArgsOnly(nm, getReflClass(ReflClass.PACKET)).invoke(nm, packet);
      return true;
    } catch (Exception e) {
      return false;
    }
  ...
  public Object createPacket(Class<?> c) throws Exception {
    Class<?> pdsC = getReflClass(ReflClass.PACKET_DATA_SERIALIZER);
    Constructor<?> ctor = pdsC.getConstructor(ByteBuf.class);
    Object buf = ctor.newInstance(Unpooled.wrappedBuffer(new byte[1024]));

    try {
      // Try to get the constructor which accepts a PDS
      // If this constructor is available, a empty default constructor isn't
      Constructor<?> packetBufC = c.getConstructor(pdsC);
      return packetBufC.newInstance(buf);
    }

    // No PDS ctor available, try the empty default constructor
    catch (Exception e) {
      Constructor<?> defC = c.getConstructor();
      defC.setAccessible(true);
      return defC.newInstance();
    }
  }
  ...
  public<T> List<? extends Class<? extends T>> findImplClasses(
    ...
    return findClasses(plugin, c ->
      ...
      // Skip self
      !impl.equals(c)
    ).stream().map(c -> (Class<? extends T>) c).collect(Collectors.toList());
  }
  ...
  public Class<?> getReflClass(ReflClass rc) {
    ...
    try {
      c = rc.resolve(refactoredNetwork, ver);
      reflClasses.put(rc, c);
      return c;
    } catch (Exception e) {
      e.printStackTrace();
      return null;
  ...
  public Optional<Integer> getBurnTime(Material mat) {
    ...
    try {
      Method newCraftStack = getClassBKT("inventory.CraftItemStack").getDeclaredMethod(
        "asNewCraftStack", getReflClass(ReflClass.ITEM)
      );
      ...
      Method lutGetter = Arrays.stream(getReflClass(ReflClass.TILE_ENTITY_FURNACE).getDeclaredMethods())
  ...
  public Function<Player, Integer> getWidAccess() {
    try {
      ...
      try {
        // Older versions have two Container fields (default, active)
        containerField = findFieldByType(ehC, cC, 1);
      } catch (Exception e) {
        // Where as newer versions just have the active field
        containerField = findFieldByType(ehC, cC, 0);
      }

      // Find the first public int member of the container, thats the window ID
      Field wIdF = Arrays.stream(cC.getDeclaredFields())
        .filter(f -> (
